...
var CommonBlockNode = class extends ElementNode3 {
  constructor() {
    super(...arguments);
    this.__id = "";
    this.__alignment = "";
...
var DetailsNode = class _DetailsNode extends ElementNode3 {
  ...
  createDOM(_config, _editor) {
    const el2 = document.createElement("details");
    if (this.__id) {
      el2.setAttribute("id", this.__id);
    }
    if (this.__dir) {
      el2.setAttribute("dir", this.__dir);
    }
    if (this.__open) {
      el2.setAttribute("open", "true");
    }
...
var TableNode4 = class _TableNode extends CommonBlockNode {
  ...
  updateDOM(_prevNode) {
    return commonPropertiesDifferent(_prevNode, this) || this.__colWidths.join(":") !== _prevNode.__colWidths.join(":") || this.__styles.size !== _prevNode.__styles.size || Array.from(this.__styles.values()).join(":") !== Array.from(_prevNode.__styles.values()).join(":");
  }
  exportDOM(editor) {
    return {
...
function $createMediaNodeFromHtml(html) {
  ...
  return domElementToNode(tag, el2);
}
var urlPatterns = [
  {
...
var CaptionNode = class _CaptionNode extends ElementNode3 {
  static getType() {
    return "caption";
  }
  static clone(node) {
...
function handleDropdown(options) {
  ...
  menu.parentElement?.addEventListener("mouseleave", (event) => {
...
var highlightColorAction = (color, context) => colorAction(context, "color", color);
...
function getDetailsToolbarContent() {
  return [
...
function registerKeyboardHandling(context) {
  ...
  const unregisterEnter = context.editor.registerCommand(KEY_ENTER_COMMAND, (event) => {
    return insertAfterSingleSelectedNode(context.editor, event) || moveAfterDetailsOnEmptyLine(context.editor, event);
  }, COMMAND_PRIORITY_LOW);
